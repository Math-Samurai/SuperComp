...
double calc_snorm(double *vec, int32_t size)
{
    double res = 0;
    for (int32_t i = 0; i < size; ++i)
    {
...
double **create_matrix_A(double *right, int32_t node_count_on_axis, double step)
{
    ...
    for (int32_t node_index = 0, row_index = 0; node_index < node_count_on_axis * node_count_on_axis && row_index < (node_count_on_axis - 2) * (node_count_on_axis - 2); ++node_index)
    {
...
double *solve_system(double **mx, double *right, int32_t size, double delta)
{
    ...
    while (1)
    {
        ...
        double diff_norm = calc_snorm(sol_diff, size);
